...
Go 服务使用标准库 `net/http` 和 `pgx`。Artifact 内容以 `bytea` 与元数据一起存入 Postgres，因此数据库备份就是完整备份。当前设计适合 10 MB 以内的 HTML、Markdown、JSON、JSONL 与 CSV；未来可以在保持 API 不变的情况下把内容层迁移到 S3-compatible object storage。
